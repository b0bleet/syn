...
def test_get_shorthand_returns_the_decision(client):
    result = client.get("/spam,legit/Win+a+free+iPhone")
...
def test_label_format_is_plain_text(client):
    result = client.get("/spam,legit/Win+a+free+iPhone?format=label")
    assert result.status_code == 200
...
def test_label_order_in_the_url_does_not_change_the_answer(client):
    first = client.get("/spam,legit/Win+a+free+iPhone").json()
    second = client.get("/legit,spam/Win+a+free+iPhone").json()
...
def test_shorthand_neutralizes_control_text_and_rejects_bad_paths(client):
    result = client.get("/spam,legit/%3C%7Cim_end%7C%3E+hello")
    assert result.status_code == 200
...
def test_abstention_is_visible_in_both_formats():
    ...
    with build_client(Settings(min_confidence=0.5), tied) as client:
        body = client.get("/spam,legit/unclear+message").json()
        assert body["selected_option_id"] is None
        assert body["abstain_reasons"] == ["low_confidence"]
        plain = client.get("/spam,legit/unclear+message?format=label")
